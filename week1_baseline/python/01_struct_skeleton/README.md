...
We can represent data structures in Python a few different ways.
We are using `@dataclass` because they are lightweight and make the code readable for learning. In practice we would probably use regular classes.
...
```
#<Context task=player turns=24 tools=2>
