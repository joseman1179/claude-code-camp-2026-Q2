...
module Boukensha
  ...
  # the full transcript.
  #
  # Options are the same as Boukensha.run, minus `task` (the user supplies tasks
  ...
  def self.repl(
    ...
    be = case backend
         when :anthropic    then Backends::Anthropic.new(api_key: api_key, model: model)
         when :deepseek      then Backends::DeepSeek.new(api_key: api_key, model: model)
         when :openai       then Backends::OpenAI.new(api_key: api_key, model: model)
         when :gemini       then Backends::Gemini.new(api_key: api_key, model: model)
         when :ollama       then Backends::Ollama.new(host: ollama_host, model: model)
         when :ollama_cloud then Backends::OllamaCloud.new(api_key: api_key, model: model)
         else raise ArgumentError, "Unknown backend #{backend.inspect}. Use :anthropic, :deepseek, :openai, :gemini, :ollama, or :ollama_cloud."
         end
...
require_relative "boukensha/backends/deepseek"
